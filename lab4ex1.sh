...
# Prints the Name and Major for each entry, excludes first line of header info with counter
...
# Loop through each line in the file, print Name and Major for each line, excludes first line with header info
while read name year major gpa state
do
	# Increment the count variable by one
	count=`expr $count + 1`

	# If not the first line, print Name and Major
	if [ $count -ne 1 ]
	then
